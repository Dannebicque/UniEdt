...
async function handleLogin() {
  try {
    const success = await $fetch('/api/auth/auth', {
      method: 'POST',
      body: credentials
    })
    // Redirige ou mets à jour l’état connecté ici
    if (success) {
      ...
      <div class="flex flex-col gap-6 w-full">
        <div class="flex flex-col gap-2 w-full">
          ...
          <InputText id="email1"
                     ...
                     type="text" placeholder="Email address" class="w-full px-3 py-2 shadow-sm rounded-lg" />
        </div>
        <div class="flex flex-col gap-2 w-full">
          <label for="password1" class="text-surface-900 dark:text-surface-0 font-medium leading-normal">Password</label>
          <InputText id="password1" type="password"
                     v-model="credentials.password"
                     placeholder="Password" class="w-full px-3 py-2 shadow-sm rounded-lg" />
        </div>
