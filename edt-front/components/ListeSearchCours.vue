...
import { fetchSearchCourses } from '~/services/courses.js'
...
const fetchFilteredCourses = async () => {
  // via un fetch pour récupérer les data de l'API, toutes semaines confondues
  let courses = await fetchSearchCourses(
    selectedSemester.value,
    selectedProfessor.value,
    selectedCourse.value,
    selectedGroup.value
  )
  // Filtre vacataires si besoin
  if (vacatairesOnly.value) {
    courses = courses.filter(course => course.isVacataire === true)
  }
  filteredCourses.value = courses
}
defineExpose({ fetchFilteredCourses })
// Watch sur chaque filtre
watch(
  [selectedSemester, selectedProfessor, selectedCourse, selectedGroup, vacatairesOnly],
...
const displayCourseListe = (course) => {
  ...
  let texte = `--${course.semaine}-- <br> ${course.matiere} <br> ${course.professor} <br> ${course.semester} <br> ${groupe}`
...
const onDragStart = (event, course, source, originSlot = '') => {
  ...
  event.dataTransfer.setData('courseSemaine', course.semaine)
...
<template>
  <div v-if="config">
    ...
    <Message severity="info">
      <strong>Info !</strong> Au moins un filtre est nécessaire pour voir les cours.
    </Message>
